...
export default class RezoDraw {
  ...
  static collectOffspringIds(def, nodeId) {
    ...
    for (const pk of Object.keys(pieces)) {
      offspringIds.push(pk)
      offspringIds.concat(RezoDraw.collectOffspringIds(def, pk))
    }
    return offspringIds
  }
  ...
  static computeDraw(rawDef, rawFacts) {
    ...
    for (const { pieceId, nodeId } of foldedList) {
      hiddenNodeIds.push(pieceId)
      hiddenNodeIds.concat(RezoDraw.collectOffspringIds(def, pieceId))
    }
