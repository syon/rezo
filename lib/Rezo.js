...
export default class Rezo {
  ...
  static judgeCompleted(def, facts, nodeKey) {
    ...
    if (!nodePieces || Object.keys(nodePieces).length === 0) {
      const judgeByNodeTitle = facts.includes(nodeTitle)
  ...
  static extractFacts(root) {
    ...
    const factsAll = Object.entries(boxes)
      ...
      .flat()
    return [...new Set(factsAll)]
  }

  static ADD_FACT(fact) {
    _facts.push(fact)
    _facts = [...new Set(_facts)]
  }

  static REM_FACT(fact) {
    _facts = _facts.filter((title) => title !== fact)
    _facts = [...new Set(_facts)]
  }
}
